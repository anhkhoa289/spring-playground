...
/**
 * Service for manager authentication operations
 * Handles manager registration, login, and token management using Manager Keycloak instance
 */
@Service
@Slf4j
...
public class ManagerAuthService {
	...
	private final IIAMManagerService iamManagerService;

	private final RestTemplate restTemplate = new RestTemplate();

	private final ObjectMapper objectMapper = new ObjectMapper();

	@Value("${keycloak.manager.auth-server-url}")
	private String authServerUrl;

	@Value("${keycloak.manager.realm}")
	private String realm;

	@Value("${keycloak.manager.client-id}")
	private String clientId;

	@Value("${keycloak.manager.credentials.secret}")
	...
	public AuthResponse register(String username, String email, String firstName, String lastName, String password) {
		log.info("Registering new manager: {}", username);

		// Create manager in Keycloak
		String userId = iamManagerService.createUser(username, email, firstName, lastName, password);

		// Login to get token
		AuthResponse authResponse = login(username, password);
		authResponse.setUserId(userId);

		log.info("Manager registered successfully: {}", username);
		return authResponse;
	}
	...
	public AuthResponse login(String username, String password) {
		log.info("Manager login attempt: {}", username);
		...
		try {
			...
			log.info("Manager logged in successfully: {}", username);
			return new AuthResponse(accessToken, refreshToken, expiresIn);
		}
		catch (Exception e) {
			log.error("Login failed for manager: {}", username, e);
			throw new RuntimeException("Invalid username or password", e);
		}
	}

	/**
	 * Get current manager information from access token
	 *
	 * @param token Access token
	 * @return UserResponse with manager information
	 */
	public UserResponse getCurrentUser(String token) {
		log.debug("Getting current manager from token");

		try {
			// Parse JWT token to get user ID
			String userId = getUserIdFromToken(token);

			// Get manager details from Keycloak
			Map<String, Object> userDetails = iamManagerService.getUserById(userId);
			List<String> roles = iamManagerService.getUserRoles(userId);
